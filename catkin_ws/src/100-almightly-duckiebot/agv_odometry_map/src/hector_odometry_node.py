...
class HectorOdometryNode(object):
	...
	def __init__(self): 
		...
		self.wheel_state_record = Point()
		self.wheel_state = Point()
	...
	def drawOdometry(self):
		...
		point = Point(self.wheel_state.trans[0], self.wheel_state.trans[1], self.wheel_state.trans[2])
		odom.pose.pose = Pose(point, self.wheel_state.rot)

		if self.wheel_state_record.time is not None:
